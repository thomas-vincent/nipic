...
def linear_regression(df, predictor, outcome, covariables=None, interactions=None,
                      ...
                      make_plots=False, fig_dir=None, report_dir=None, file_suffix='',
                      outlier_qthresh=None):

    if outlier_qthresh is not None:
        q1 = df[outcome].quantile(outlier_qthresh)
        q3 = df[outcome].quantile(1-outlier_qthresh)
        iqr = q3 - q1
        m_outliers = ((df[outcome] < (q1 - 1.5 * iqr)) |
                      (df[outcome] > (q3 + 1.5 * iqr)))
        if m_outliers.sum() > 0:
            logger.info('Filter %d outliers', m_outliers.sum())
            df = df[~m_outliers]
    ...
    cls = LinearRegDiagnostic(estimate)
    vif_table, fig_diagn, ax_diagn = cls(high_leverage_threshold=True)
    fig_diagn.tight_layout(pad=1.0)
    ...
    estimate.influencials = pd.DataFrame(False, index=df.index, columns=all_indep_vars)
    for indep_var in all_indep_vars:
        ...
        if nb_influencial > 0:
            logger.debug('Influencial obs (n=%d) for %s', nb_influencial, indep_var)

        estimate.influencials.loc[m_influence, indep_var] = True
    ...
    diagnoses = pd.Series([estimate.diagn['omni'], estimate.diagn['omnipv'],
                           estimate.condition_number],
                          index=['Omnibus_chi2', 'Omnibus_pvalue', 'Condition_Number'])
    ...
    if make_plots:
        ...
        if fig_dir is not None:
            ...
            fig_diagn.savefig(op.join(fig_dir, '%s_corr_%s_%s%s_diagnosis.png' %
                                      (predictor, outcome, '_'.join(covariables), file_suffix)))
...
class LinearRegDiagnostic:
    ...
    def __init__(self, results):
        ...
        self.residual_norm = self.influence.resid_studentized_internal
    ...
    def leverage_plot(
        ...
        dbetas = self.influence.summary_frame().filter(regex="dfb")
        influence_threshold = 2.0 / self.nresids ** 0.5
        enames = self.results.model.exog_names
        pred_var = enames[0] if enames[0] != 'const' else enames[1]
        dfb_col = 'dfb_' + pred_var
        m_influence = dbetas[dfb_col].abs() > influence_threshold
        # embed()
        for idx in dbetas.index[m_influence]:
            i = dbetas.index.get_loc(idx)
            ax.annotate(idx, xy=(self.leverage[i], self.residual_norm[i]), color="darkorange")
